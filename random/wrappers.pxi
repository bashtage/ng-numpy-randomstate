...
cdef object cont0(aug_state* state, random_double_0 func, object size, object lock):
    if size is None:
        return func(state)
...
cdef object cont1(aug_state* state, random_double_1 func, double a, object size, object lock):
    if size is None:
        return func(state, a)
    cdef Py_ssize_t i, n = compute_numel(size)
    cdef double [:] randoms = np.empty(n, np.double)
...
cdef object cont2(aug_state* state, random_double_2 func, double a, double b, object size, object lock):
    if size is None:
...
cdef object cont3(aug_state* state, random_double_3 func, double a, double b, double c, object size, object lock):
    if size is None:
...
cdef object uint0(aug_state* state, random_uint_0 func, object size, object lock):
    if size is None:
        return func(state)
    cdef Py_ssize_t i, n = compute_numel(size)
